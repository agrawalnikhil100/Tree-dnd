...
describe('HighlightedText/Helpers', function () {
    describe('calculateWordHighlights', function () {
        it('correctly extracts indices given overlaps', function () {
            var result = Helpers_1.calculateWordHighlights('hi hihih', ['hi', 'hih']).sort(sortArray);
            var expected = [
                // match "hi"
                {
                    startIndex: 0,
        ...
        it('correctly handles multi-byte characters', function () {
            var result = Helpers_1.calculateWordHighlights('🤠 hi 🤠', ['hi', '🤠']).sort(sortArray);
            var expected = [
        ...
        it('is case sensitive by default', function () {
            var result = Helpers_1.calculateWordHighlights('Hi hi', ['hi']).sort(sortArray);
            var expected = [{ startIndex: 3, length: 2 }].sort(sortArray);
            expect(result).toEqual(expected);
        });
        it('allows for overriding case sensitivity', function () {
            {
                var result = Helpers_1.calculateWordHighlights('Hi hi', ['hi'], true).sort(sortArray);
                var expected = [
                    { startIndex: 0, length: 2 },
                    { startIndex: 3, length: 2 }
                ].sort(sortArray);
            ...
            {
                var result = Helpers_1.calculateWordHighlights('Hi hi', ['hi'], false).sort(sortArray);
                var expected = [{ startIndex: 3, length: 2 }].sort(sortArray);
                expect(result).toEqual(expected);
            }
        });
