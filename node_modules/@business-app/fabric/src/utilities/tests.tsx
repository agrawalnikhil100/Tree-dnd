...
export function simulateInput<T>(
  Component: React.ComponentType<IInputComponentProps<T>>,
  baseValue: T,
  ...valuesToInput: string[]
): { hasBeenChanged: boolean; lastValue?: T } {
  const onChangedSpy = jest.fn();

  const element = <Component value={baseValue} onChanged={onChangedSpy} />;

  const mounted = mount(element);
  const inputElement = mounted.find('input');

  valuesToInput.forEach(value => {
    inputElement.simulate('focus');
    inputElement.simulate('input', { target: { value } });
    inputElement.simulate('blur');
  });

  const callCount = onChangedSpy.mock.calls.length;
