/*!
 * Copyright (C) Microsoft Corporation. All rights reserved.
 */

import { mount, ReactWrapper } from 'enzyme';
import { IContextualMenuProps } from '@fluentui/react/lib/ContextualMenu';
import { KeyCodes } from '@fluentui/react/lib/Utilities';
import * as React from 'react';
import { ContextualMenuButton } from './ContextualMenuButton/ContextualMenuButton';
import { Tree } from './Tree';
import { ITreeState } from './Tree.base';
import { TreeAutomationSelectors } from './Tree.selectors';
import { ITreeItem, ITreeProps, SelectionType } from './Tree.types';
import { TreeNode } from './TreeNode/TreeNode';

interface ITestItem extends ITreeItem {
  testPropertyA: number;
  testPropertyB: string;
}

type TreeProps = ITreeProps<ITestItem>;
type TreeState = ITreeState<ITestItem>;

type TreeWrapper = ReactWrapper<ITreeProps<ITestItem>, TreeState>;

function getStateOf(wrapper: TreeWrapper): TreeState {
  const instance = wrapper.childAt(0).childAt(0).instance();

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return (instance as any).state;
}

function getDefaultProps(overrides?: Partial<TreeProps>): TreeProps {
  return {
    items: getMockItems([{}, {}]),
    strings: {
      listAriaLabel: 'list aria label',
      moreCommandsAriaLabel: 'more commands aria label'
    },
    ...overrides
  };
}

type MockItemConstructionOption = {
  children?: MockItemConstructionOption[];
} & Partial<ITestItem>;

function getMockItems(
  items: MockItemConstructionOption[],
  treeItems: ITestItem[] = [],
  level: number = 0,
  parentIndex: number = -1
): ITestItem[] {
  items.forEach((item, indexInParent) => {
    const { children, ...restItem } = item;
    const itemId = `${level} ${parentIndex} ${indexInParent}`;

    // Add the item
    treeItems.push({
      label: itemId,
      key: itemId,
      isFolder: !!children,
      level: level,
      setSize: items.length,
      indexInParent: indexInParent,
      testPropertyA: level * 100 + indexInParent,
      testPropertyB: itemId,
      ...restItem
    });

    // If the item is a category and has children, add the children
    if (item.children) {
      getMockItems(item.children, treeItems, level + 1, indexInParent);
    }
  });

  return treeItems;
}

describe('Tree', () => {
  describe('Collapse/Expand', () => {
    it('initialExpandedItemKeys', () => {
      const props = getDefaultProps({
        items: getMockItems([
          {
            key: 'one',
            children: [{ key: 'two' }]
          },
          {
            key: 'three',
            children: [{ key: 'four' }]
          }
        ]),
        initialExpandedItemKeys: new Set(['three'])
      });
      const wrapper: TreeWrapper = mount(<Tree {...props} />);

      expect(getStateOf(wrapper).expandedItemKeys.has('three')).toBeTruthy();
      expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);

      wrapper.unmount();
    });

    it('expandedItemKeys has higher precedence than initialExpandedItemKeys', () => {
      const props = getDefaultProps({
        items: getMockItems([
          {
            key: 'one',
            children: [
              {
                key: 'two',
                children: [{ key: 'three' }]
              }
            ]
          },
          {
            key: 'four',
            children: [{ key: 'five' }]
          }
        ]),
        initialExpandedItemKeys: new Set(['four']),
        expandedItemKeys: new Set(['two'])
      });
      const wrapper: TreeWrapper = mount(<Tree {...props} />);

      expect(getStateOf(wrapper).expandedItemKeys.has('one')).toBeFalsy();
      expect(getStateOf(wrapper).expandedItemKeys.has('two')).toBeTruthy();
      expect(getStateOf(wrapper).expandedItemKeys.has('four')).toBeFalsy();

      wrapper.unmount();
    });

    it('TreeNodes should only be rendered if parents are expanded', () => {
      const items: ITestItem[] = getMockItems([
        {
          key: 'one',
          children: [
            {
              key: 'two',
              children: [{ key: 'three' }]
            },
            { key: 'four' },
            { key: 'five' }
          ]
        },
        {
          key: 'six',
          children: [{ key: 'seven' }]
        },
        { key: 'eight' }
      ]);
      const props = getDefaultProps({ items, expandedItemKeys: new Set() });
      const wrapper: TreeWrapper = mount(<Tree {...props} />);

      // Child node should be hidden since parent is collapsed by default.
      expect(wrapper.find(TreeNode).length).toEqual(3);

      wrapper.setProps({ expandedItemKeys: new Set(['one']) });

      // Child node should exist now that parent is expanded.
      expect(wrapper.find(TreeNode).length).toEqual(6);

      wrapper.setProps({ expandedItemKeys: new Set(['one', 'two']) });

      // Expand inner folder, too.
      expect(wrapper.find(TreeNode).length).toEqual(7);

      wrapper.setProps({ expandedItemKeys: new Set(['two']) });

      // Collapse out (inner expanded, but not shown because outer).
      expect(wrapper.find(TreeNode).length).toEqual(3);

      wrapper.setProps({ expandedItemKeys: new Set(['one', 'two']) });

      // Expand outer again, inner still expanded.
      expect(wrapper.find(TreeNode).length).toEqual(7);

      wrapper.unmount();
    });
  });

  describe('Selection', () => {
    it('initialSelectedItemKeys', () => {
      const props = getDefaultProps({
        items: getMockItems([
          { key: 'one' },
          { key: 'two' },
          { key: 'three' },
          { key: 'four' },
          { key: 'five' },
          { key: 'six' }
        ]),
        initialSelectedItemKeys: new Set(['two', 'four'])
      });
      const wrapper: TreeWrapper = mount(<Tree {...props} />);

      expect(getStateOf(wrapper).selectedItemKeys.has('two')).toBeTruthy();
      expect(getStateOf(wrapper).selectedItemKeys.has('four')).toBeTruthy();
      expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(2);

      wrapper.unmount();
    });

    it('selectedItemKeys has higher precedence than initialSelectedItemKeys', () => {
      const props = getDefaultProps({
        items: getMockItems([
          { key: 'one' },
          { key: 'two' },
          { key: 'three' },
          { key: 'four' },
          { key: 'five' },
          { key: 'six' }
        ]),
        initialSelectedItemKeys: new Set(['two', 'four']),
        selectedItemKeys: new Set(['three', 'five'])
      });
      const wrapper: TreeWrapper = mount(<Tree {...props} />);

      expect(getStateOf(wrapper).selectedItemKeys.has('one')).toBeFalsy();
      expect(getStateOf(wrapper).selectedItemKeys.has('two')).toBeFalsy();
      expect(getStateOf(wrapper).selectedItemKeys.has('three')).toBeTruthy();
      expect(getStateOf(wrapper).selectedItemKeys.has('four')).toBeFalsy();
      expect(getStateOf(wrapper).selectedItemKeys.has('five')).toBeTruthy();
      expect(getStateOf(wrapper).selectedItemKeys.has('six')).toBeFalsy();

      wrapper.unmount();
    });

    it('selectedItemKeys', () => {
      const props = getDefaultProps({
        items: getMockItems([
          { key: 'one' },
          { key: 'two' },
          { key: 'three' },
          { key: 'four' },
          { key: 'five' },
          { key: 'six' }
        ]),
        selectedItemKeys: new Set()
      });
      const wrapper: TreeWrapper = mount(<Tree {...props} />);

      expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(0);
      expect(wrapper.find(TreeNode).at(0).getDOMNode().getAttribute('aria-selected')).toEqual('false');

      wrapper.setProps({ selectedItemKeys: new Set(['two']) });
      expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
      expect(getStateOf(wrapper).selectedItemKeys.has('two')).toBeTruthy();
      expect(wrapper.find(TreeNode).at(1).getDOMNode().getAttribute('aria-selected')).toEqual('true');

      wrapper.setProps({ selectedItemKeys: new Set(['three']) });
      expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
      expect(getStateOf(wrapper).selectedItemKeys.has('three')).toBeTruthy();
      expect(wrapper.find(TreeNode).at(2).getDOMNode().getAttribute('aria-selected')).toEqual('true');

      wrapper.setProps({ selectedItemKeys: new Set(['three', 'five']) });
      expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(2);
      expect(getStateOf(wrapper).selectedItemKeys.has('three')).toBeTruthy();
      expect(getStateOf(wrapper).selectedItemKeys.has('five')).toBeTruthy();
      expect(wrapper.find(TreeNode).at(2).getDOMNode().getAttribute('aria-selected')).toEqual('true');
      expect(wrapper.find(TreeNode).at(4).getDOMNode().getAttribute('aria-selected')).toEqual('true');

      wrapper.setProps({ selectedItemKeys: new Set() });
      expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(0);
      expect(wrapper.find(TreeNode).at(2).getDOMNode().getAttribute('aria-selected')).toEqual('false');
      expect(wrapper.find(TreeNode).at(4).getDOMNode().getAttribute('aria-selected')).toEqual('false');

      wrapper.unmount();
    });

    it('Selection changes should result in node re-renders when necessary', () => {
      const onRenderItemLabel = jest.fn();
      const props = getDefaultProps({
        items: getMockItems([{ key: 'one' }, { key: 'two' }]),
        selectedItemKeys: new Set(),
        onRenderItemLabel
      });
      const wrapper: TreeWrapper = mount(<Tree {...props} />);

      const isNodeSelected = (index: number) => wrapper.find(TreeNode).at(index).props().isSelected;

      expect(isNodeSelected(0)).toEqual(false);
      expect(isNodeSelected(1)).toEqual(false);
      expect(onRenderItemLabel).toBeCalledTimes(2);

      onRenderItemLabel.mockReset();
      wrapper.setProps({ selectedItemKeys: new Set(['two']) });
      expect(isNodeSelected(0)).toEqual(false);
      expect(isNodeSelected(1)).toEqual(true);
      expect(onRenderItemLabel).toHaveBeenCalled();
    });
  });

  it('onRenderItemContainer/onRenderItemLabel/getItemStyle callback arguments', () => {
    const onRenderItemContainer = jest.fn(
      (
        item: ITestItem,
        isSelected: boolean,
        isExpanded: boolean,
        index: number,
        defaultItemRender: () => JSX.Element
      ) => {
        return defaultItemRender();
      }
    );
    const onRenderItemLabel = jest.fn();
    const getItemStyle = jest.fn();

    const assertFunction = (
      expectedItem: ITestItem,
      expectSelected: boolean,
      expectExpanded: boolean,
      expectedIndex: number
    ) => {
      expect(onRenderItemContainer.mock.calls[expectedIndex][0]).toEqual(expectedItem);
      expect(onRenderItemContainer.mock.calls[expectedIndex][1]).toEqual(expectSelected);
      expect(onRenderItemContainer.mock.calls[expectedIndex][2]).toEqual(expectExpanded);
      expect(onRenderItemContainer.mock.calls[expectedIndex][3]).toEqual(expectedIndex);
      expect(onRenderItemContainer.mock.calls[expectedIndex][4]).toBeDefined();

      expect(onRenderItemLabel.mock.calls[expectedIndex][0]).toEqual(expectedItem);
      expect(onRenderItemLabel.mock.calls[expectedIndex][1]).toEqual(expectSelected);
      expect(onRenderItemLabel.mock.calls[expectedIndex][2]).toEqual(expectExpanded);
      expect(onRenderItemLabel.mock.calls[expectedIndex][3]).toEqual(expectedIndex);

      expect(getItemStyle.mock.calls[expectedIndex][0]).toEqual(expectedItem);
      expect(getItemStyle.mock.calls[expectedIndex][1]).toEqual(expectSelected);
      expect(getItemStyle.mock.calls[expectedIndex][2]).toEqual(expectExpanded);
      expect(getItemStyle.mock.calls[expectedIndex][3]).toEqual(expectedIndex);
    };

    const items = getMockItems([
      { key: 'one' },
      { key: 'two' },
      {
        key: 'three',
        children: [{ key: 'four' }]
      },
      {
        key: 'five',
        children: [{ key: 'six' }]
      }
    ]);

    const props = getDefaultProps({
      items: items,
      selectedItemKeys: new Set(['two', 'five']),
      expandedItemKeys: new Set(['three', 'five']),
      onRenderItemContainer: onRenderItemContainer,
      onRenderItemLabel: onRenderItemLabel,
      getItemStyle: getItemStyle
    });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    assertFunction(items[0], false, false, 0);
    assertFunction(items[1], true, false, 1);
    assertFunction(items[2], false, true, 2);
    assertFunction(items[3], false, false, 3);
    assertFunction(items[4], true, true, 4);
    assertFunction(items[5], false, false, 5);
    expect(onRenderItemContainer).toHaveBeenCalledTimes(6);
    expect(onRenderItemLabel).toHaveBeenCalledTimes(6);

    wrapper.unmount();
  });

  it('Should render labels when no onRenderItemLabel is provided', () => {
    const items = getMockItems([{ label: 'one' }, { label: 'two' }]);

    const props = getDefaultProps({ items });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(wrapper.find(TreeNode).at(0).getDOMNode().innerHTML.indexOf('one')).toBeGreaterThan(-1);
    expect(wrapper.find(TreeNode).at(1).getDOMNode().innerHTML.indexOf('two')).toBeGreaterThan(-1);

    wrapper.unmount();
  });

  it('onRenderItemContainer should work', () => {
    type ICustomItem = ITreeItem & { customProperty: string };
    const wrappingClassName = 'new-class-for-test';
    const wrappingClassNameSelector = `.${wrappingClassName}`;

    const onRenderItemContainer = (
      item: ICustomItem,
      isSelected: boolean,
      isExpanded: boolean,
      index: number,
      defaultItemRender: () => JSX.Element
    ) => {
      return <div className={wrappingClassName}>{defaultItemRender()}</div>;
    };
    const wrapper: TreeWrapper = mount(
      <Tree
        items={
          [
            {
              label: 'label',
              key: 'key',
              level: 0,
              setSize: 1,
              indexInParent: 0,
              customProperty: 'pizza'
            }
          ] as ICustomItem[]
        }
        strings={{
          listAriaLabel: 'list',
          moreCommandsAriaLabel: 'more commands'
        }}
        onRenderItemContainer={onRenderItemContainer}
      />
    );

    const numberOfNodes = wrapper.find(TreeNode).length;
    expect(numberOfNodes).toBeGreaterThan(0);

    expect(wrapper.find(wrappingClassNameSelector).length).toEqual(numberOfNodes);
    wrapper.setProps({ onRenderItemContainer: undefined });
    expect(wrapper.find(wrappingClassNameSelector).length).toEqual(0);
    expect(wrapper.find(TreeNode).length).toBeGreaterThan(0);

    wrapper.unmount();
  });

  it('onRenderItemLabel should work and allow access to custom item properties', () => {
    type ICustomItem = ITreeItem & { customProperty: string };
    const onRenderItemLabel = (item: ICustomItem) => {
      return <span>{item.customProperty}</span>;
    };
    const wrapper: TreeWrapper = mount(
      <Tree
        items={
          [
            {
              label: 'label',
              key: 'key',
              level: 0,
              setSize: 1,
              indexInParent: 0,
              customProperty: 'pizza'
            }
          ] as ICustomItem[]
        }
        strings={{
          listAriaLabel: 'list',
          moreCommandsAriaLabel: 'more commands'
        }}
        onRenderItemLabel={onRenderItemLabel}
      />
    );

    expect(wrapper.find(TreeNode).at(0).getDOMNode().innerHTML.indexOf('pizza')).toBeGreaterThan(-1);

    wrapper.unmount();
  });

  it('onRenderItemLabelContent should work and allow access to custom item properties', () => {
    type ICustomItem = ITreeItem & { customProperty: string };
    const onRenderItemLabelContent = (item: ICustomItem) => {
      return <span>{item.customProperty}</span>;
    };
    const wrapper: TreeWrapper = mount(
      <Tree
        items={
          [
            {
              label: 'label',
              key: 'key',
              level: 0,
              setSize: 1,
              indexInParent: 0,
              customProperty: 'pizza'
            }
          ] as ICustomItem[]
        }
        strings={{
          listAriaLabel: 'list',
          moreCommandsAriaLabel: 'more commands'
        }}
        onRenderItemLabelContent={onRenderItemLabelContent}
      />
    );

    expect(wrapper.find(TreeNode).at(0).getDOMNode().innerHTML.indexOf('pizza')).toBeGreaterThan(-1);

    wrapper.unmount();
  });

  it('onRenderExpandIcon should work and allow access to custom item properties', () => {
    type ICustomItem = ITreeItem & { customProperty: string };
    const onRenderExpandIcon = (item: ICustomItem) => {
      return <span>{item.customProperty}</span>;
    };
    const wrapper: TreeWrapper = mount(
      <Tree
        items={
          [
            {
              label: 'label',
              key: 'key',
              level: 0,
              setSize: 1,
              indexInParent: 0,
              customProperty: 'pizza'
            }
          ] as ICustomItem[]
        }
        strings={{
          listAriaLabel: 'list',
          moreCommandsAriaLabel: 'more commands'
        }}
        onRenderExpandIcon={onRenderExpandIcon}
      />
    );

    expect(wrapper.find(TreeNode).at(0).getDOMNode().innerHTML.indexOf('pizza')).toBeGreaterThan(-1);

    wrapper.unmount();
  });

  it('Uncontrolled onItemExpand/onItemCollapse', () => {
    const onItemExpand = jest.fn();
    const onItemCollapse = jest.fn();
    const collapseExpandSelector = `.${TreeAutomationSelectors.collapseExpandClassName}`;

    const items = getMockItems([
      {
        key: 'one',
        children: [
          {
            key: 'two',
            children: [{ key: 'three' }]
          }
        ]
      },
      {
        key: 'four',
        children: [{ key: 'five' }]
      }
    ]);

    const props = getDefaultProps({
      items: items,
      initialExpandedItemKeys: new Set(['four']),
      onItemExpand: onItemExpand,
      onItemCollapse: onItemCollapse
    });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.has('four')).toEqual(true);

    // Expand the first node
    wrapper.find(collapseExpandSelector).at(0).simulate('click');
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(2);
    expect(getStateOf(wrapper).expandedItemKeys.has('one')).toEqual(true);
    expect(onItemExpand).toHaveBeenCalledTimes(1);
    expect(onItemExpand).toHaveBeenLastCalledWith(items[0], 0);

    // Expand the first node's child node
    wrapper.find(collapseExpandSelector).at(1).simulate('click');
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(3);
    expect(getStateOf(wrapper).expandedItemKeys.has('one')).toEqual(true);
    expect(getStateOf(wrapper).expandedItemKeys.has('two')).toEqual(true);
    expect(onItemExpand).toHaveBeenCalledTimes(2);
    expect(onItemExpand).toHaveBeenLastCalledWith(items[1], 1);

    // Collapse the first node
    wrapper.find(collapseExpandSelector).at(0).simulate('click');
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(2);
    expect(getStateOf(wrapper).expandedItemKeys.has('one')).toEqual(false);
    expect(getStateOf(wrapper).expandedItemKeys.has('two')).toEqual(true);
    expect(getStateOf(wrapper).expandedItemKeys.has('four')).toEqual(true);
    expect(onItemCollapse).toHaveBeenCalledTimes(1);
    expect(onItemCollapse).toHaveBeenLastCalledWith(items[0], 0);

    wrapper.find(collapseExpandSelector).at(1).simulate('click');
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.has('two')).toEqual(true);
    expect(getStateOf(wrapper).expandedItemKeys.has('four')).toEqual(false);
    expect(onItemCollapse).toHaveBeenCalledTimes(2);
    expect(onItemCollapse).toHaveBeenLastCalledWith(items[3], 1);

    wrapper.unmount();
  });

  it('Controlled onItemExpand/onItemCollapse', () => {
    const onItemExpand = jest.fn();
    const onItemCollapse = jest.fn();
    const collapseExpandSelector = `.${TreeAutomationSelectors.collapseExpandClassName}`;

    const items = getMockItems([
      {
        key: 'one',
        children: [
          {
            key: 'two',
            children: [{ key: 'three' }]
          }
        ]
      },
      {
        key: 'four',
        children: [{ key: 'five' }]
      }
    ]);

    const props = getDefaultProps({
      items: items,
      expandedItemKeys: new Set(['four']),
      onItemExpand: onItemExpand,
      onItemCollapse: onItemCollapse
    });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.has('four')).toEqual(true);

    // Expand the first node (but it won't actually expand because it's controlled)
    wrapper.find(collapseExpandSelector).at(0).simulate('click');
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.has('four')).toEqual(true);
    expect(onItemExpand).toHaveBeenCalledTimes(1);
    expect(onItemExpand).toHaveBeenLastCalledWith(items[0], 0);

    // Collapse the fourth node (but it won't actually collapse because it's controlled)
    wrapper.find(collapseExpandSelector).at(1).simulate('click');
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.has('four')).toEqual(true);
    expect(onItemCollapse).toHaveBeenCalledTimes(1);
    expect(onItemCollapse).toHaveBeenLastCalledWith(items[3], 1);

    wrapper.unmount();
  });

  it('onItemDoubleClicked', () => {
    const onItemDoubleClicked = jest.fn();
    const onItemSelected = jest.fn();
    const collapseExpandSelector = `.${TreeAutomationSelectors.collapseExpandClassName}`;

    const items = getMockItems([
      {
        key: 'one',
        children: [{ key: 'two' }]
      },
      { key: 'three' },
      {
        key: 'four',
        children: [{ key: 'five' }]
      },
      {
        key: 'six',
        children: [{ key: 'seven' }]
      }
    ]);

    const props = getDefaultProps({
      items: items,
      expandedItemKeys: new Set(['four', 'six']),
      selectedItemKeys: new Set(['three', 'six']),
      onItemDoubleClicked: onItemDoubleClicked,
      onItemSelected: onItemSelected,
      onRenderItemLabel: () => null,
      onItemExpand: () => undefined,
      onItemCollapse: () => undefined,
      getItemCommands: () => []
    });

    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    // Double click a not selected and not expanded node.
    wrapper.find(TreeNode).at(0).simulate('doubleclick');
    expect(onItemDoubleClicked).toHaveBeenCalledTimes(1);
    expect(onItemDoubleClicked).toHaveBeenLastCalledWith(items[0], false, false, 0);
    // Double clicking should also trigger onItemSelected
    expect(onItemSelected).toHaveBeenCalledTimes(0);

    // Double click a selected but not expanded node.
    wrapper.find(TreeNode).at(1).simulate('doubleclick');
    expect(onItemDoubleClicked).toHaveBeenCalledTimes(2);
    expect(onItemDoubleClicked).toHaveBeenLastCalledWith(items[2], true, false, 1);
    expect(onItemSelected).toHaveBeenCalledTimes(0);

    // Double click an expanded but not selected node.
    wrapper.find(TreeNode).at(2).simulate('doubleclick');
    expect(onItemDoubleClicked).toHaveBeenCalledTimes(3);
    expect(onItemDoubleClicked).toHaveBeenLastCalledWith(items[3], false, true, 2);
    expect(onItemSelected).toHaveBeenCalledTimes(0);

    // Double click a node that is both selected and expanded.
    wrapper.find(TreeNode).at(4).simulate('doubleclick');
    expect(onItemDoubleClicked).toHaveBeenCalledTimes(4);
    expect(onItemDoubleClicked).toHaveBeenLastCalledWith(items[5], true, true, 4);
    expect(onItemSelected).toHaveBeenCalledTimes(0);

    onItemDoubleClicked.mockReset();
    onItemSelected.mockReset();

    // Double clicking the expand/collapse icon should not trigger the onItemDoubleClicked
    wrapper.find(collapseExpandSelector).at(0).simulate('doubleclick');
    expect(onItemDoubleClicked).toHaveBeenCalledTimes(0);
    expect(onItemSelected).toHaveBeenCalledTimes(0);

    // Double clicking the contextual menu button should not trigger the onItemDoubleClicked
    wrapper.find(ContextualMenuButton).at(0).simulate('doubleclick');
    expect(onItemDoubleClicked).toHaveBeenCalledTimes(0);
    expect(onItemSelected).toHaveBeenCalledTimes(0);

    wrapper.unmount();
  });

  it('moreCommandsAriaLabel', () => {
    const items = getMockItems([
      {
        key: 'one',
        children: [{ key: 'two' }],
        moreCommandsAriaLabel: 'commands one'
      },
      { key: 'three' },
      {
        key: 'four',
        children: [{ key: 'five' }],
        moreCommandsAriaLabel: 'commands four'
      },
      {
        key: 'six',
        children: [{ key: 'seven' }]
      }
    ]);

    const props = getDefaultProps({
      items: items,
      expandedItemKeys: new Set(['four', 'six']),
      selectedItemKeys: new Set(['three', 'six']),
      getItemCommands: () => [],
      strings: {
        listAriaLabel: 'list',
        moreCommandsAriaLabel: 'default commands'
      }
    });

    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    // It should use the moreCommandsAriaLabel for the item, if provided, and the global one otherwise.
    expect(wrapper.find(TreeNode).at(0).props().moreCommandsAriaLabel).toEqual('commands one');
    expect(wrapper.find(TreeNode).at(1).props().moreCommandsAriaLabel).toEqual('default commands');
    expect(wrapper.find(TreeNode).at(2).props().moreCommandsAriaLabel).toEqual('commands four');
    expect(wrapper.find(TreeNode).at(3).props().moreCommandsAriaLabel).toEqual('default commands');

    wrapper.unmount();
  });

  it('Uncontrolled onItemSelected', () => {
    const onItemSelected = jest.fn();

    const items = getMockItems([
      { key: 'one' },
      { key: 'two' },
      { key: 'three' },
      { key: 'four' },
      { key: 'five', disableSelection: true }
    ]);

    const props = getDefaultProps({
      items: items,
      onItemSelected: onItemSelected
    });

    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(0);

    // Unselectable nodes should not be selectable
    wrapper.find(TreeNode).at(4).simulate('click');
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(0);

    wrapper.find(TreeNode).at(0).simulate('click');
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('one')).toEqual(true);

    wrapper.find(TreeNode).at(1).simulate('click');
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);

    wrapper.find(TreeNode).at(0).simulate('click', { ctrlKey: true });
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(2);
    expect(getStateOf(wrapper).selectedItemKeys.has('one')).toEqual(true);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);

    wrapper.find(TreeNode).at(0).simulate('click', { ctrlKey: true });
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);

    wrapper.unmount();
  });

  it('Controlled onItemSelected and arguments', () => {
    const onItemSelected = jest.fn();

    let items = getMockItems([{ key: 'one' }, { key: 'two' }]);

    const props = getDefaultProps({
      items: items,
      onItemSelected: onItemSelected,
      selectedItemKeys: new Set(['two'])
    });

    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);

    wrapper.find(TreeNode).at(0).simulate('click');
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);
    expect(onItemSelected).toHaveBeenLastCalledWith(items[0], SelectionType.Single, false, false, 0);

    wrapper.find(TreeNode).at(0).simulate('click', { ctrlKey: true });
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);
    expect(onItemSelected).toHaveBeenLastCalledWith(items[0], SelectionType.Toggle, false, false, 0);

    wrapper.find(TreeNode).at(1).simulate('click');
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);
    expect(onItemSelected).toHaveBeenLastCalledWith(items[1], SelectionType.Single, true, false, 1);

    wrapper.find(TreeNode).at(1).simulate('click', { ctrlKey: true });
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).selectedItemKeys.has('two')).toEqual(true);
    expect(onItemSelected).toHaveBeenLastCalledWith(items[1], SelectionType.Toggle, true, false, 1);

    items = getMockItems([
      {
        key: 'one',
        children: [{ key: 'two' }]
      }
    ]);
    wrapper.setProps({
      items: items,
      selectedItemKeys: new Set(),
      expandedItemKeys: new Set(['one'])
    });

    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(0);
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);

    wrapper.find(TreeNode).at(0).simulate('click');
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(0);
    expect(onItemSelected).toHaveBeenLastCalledWith(items[0], SelectionType.Single, false, true, 0);

    wrapper.find(TreeNode).at(0).simulate('click', { ctrlKey: true });
    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(0);
    expect(onItemSelected).toHaveBeenLastCalledWith(items[0], SelectionType.Toggle, false, true, 0);

    wrapper.unmount();
  });

  it('onItemKeyDown arguments', () => {
    const onItemKeyDown = jest.fn();

    const items = getMockItems([
      { key: 'one' },
      { key: 'two' },
      {
        key: 'three',
        children: [{ key: 'four' }]
      }
    ]);

    const props = getDefaultProps({
      items: items,
      onItemKeyDown: onItemKeyDown,
      selectedItemKeys: new Set(['two']),
      expandedItemKeys: new Set(['three'])
    });

    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(getStateOf(wrapper).selectedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);

    wrapper.find(TreeNode).at(0).simulate('keyDown', { key: 'q' });
    let event: React.KeyboardEvent<HTMLElement> = onItemKeyDown.mock.calls[0][4];
    expect(onItemKeyDown).toHaveBeenLastCalledWith(items[0], false, false, 0, event);
    expect(event.key).toEqual('q');
    expect(event.ctrlKey).toEqual(undefined);

    onItemKeyDown.mockReset();

    wrapper.find(TreeNode).at(1).simulate('keyDown', { key: 'p', ctrlKey: true });
    event = onItemKeyDown.mock.calls[0][4];
    expect(onItemKeyDown).toHaveBeenLastCalledWith(items[1], true, false, 1, event);
    expect(event.key).toEqual('p');
    expect(event.ctrlKey).toEqual(true);

    onItemKeyDown.mockReset();

    wrapper.find(TreeNode).at(2).simulate('keyDown', { key: 'a', altKey: true });
    event = onItemKeyDown.mock.calls[0][4];
    expect(onItemKeyDown).toHaveBeenLastCalledWith(items[2], false, true, 2, event);
    expect(event.key).toEqual('a');
    expect(event.altKey).toEqual(true);

    wrapper.unmount();
  });

  it('onRenderContextualMenu arguments', () => {
    const onRenderContextualMenu = jest.fn();

    const items = getMockItems([
      { key: 'one' },
      { key: 'two' },
      {
        key: 'three',
        children: [{ key: 'four' }]
      }
    ]);

    const props = getDefaultProps({
      items: items,
      getItemCommands: () => [
        {
          key: 'New',
          text: 'New',
          iconProps: { iconName: 'Add' }
        },
        {
          key: 'Mail',
          text: 'Mail',
          iconProps: { iconName: 'Mail' }
        }
      ],
      onRenderContextualMenu: onRenderContextualMenu,
      selectedItemKeys: new Set(['two']),
      expandedItemKeys: new Set(['three'])
    });

    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(onRenderContextualMenu).toHaveBeenCalledTimes(0);

    wrapper.find(TreeNode).at(0).simulate('contextmenu');
    let menuProps: IContextualMenuProps = onRenderContextualMenu.mock.calls[0][4];
    expect(onRenderContextualMenu).toHaveBeenCalledTimes(1);
    expect(onRenderContextualMenu).toHaveBeenLastCalledWith(items[0], false, false, 0, menuProps);
    expect(menuProps.items.length).toEqual(2);

    onRenderContextualMenu.mockReset();

    wrapper.find(TreeNode).at(1).simulate('keydown', { keyCode: KeyCodes.f10, shiftKey: true });
    menuProps = onRenderContextualMenu.mock.calls[0][4];
    expect(onRenderContextualMenu).toHaveBeenCalledTimes(1);
    expect(onRenderContextualMenu).toHaveBeenLastCalledWith(items[1], true, false, 1, menuProps);
    expect(menuProps.items.length).toEqual(2);

    onRenderContextualMenu.mockReset();

    wrapper.find(TreeNode).at(2).find(ContextualMenuButton).simulate('click');
    menuProps = onRenderContextualMenu.mock.calls[0][4];
    expect(onRenderContextualMenu).toHaveBeenCalledTimes(1);
    expect(onRenderContextualMenu).toHaveBeenLastCalledWith(items[2], false, true, 2, menuProps);
    expect(menuProps.items.length).toEqual(2);

    wrapper.unmount();
  });

  it('Uncontrolled: Should be able to collapse/expand nodes via keyboard', () => {
    const items = getMockItems([
      {
        key: 'one',
        children: [
          {
            key: 'two',
            children: [{ key: 'three' }]
          }
        ]
      }
    ]);

    const props = getDefaultProps({
      items: items
    });

    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(0);

    wrapper.find(TreeNode).at(0).simulate('keyDown', { keyCode: KeyCodes.right });
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.has('one')).toEqual(true);

    wrapper.find(TreeNode).at(1).simulate('keyDown', { keyCode: KeyCodes.right });
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(2);
    expect(getStateOf(wrapper).expandedItemKeys.has('one')).toEqual(true);
    expect(getStateOf(wrapper).expandedItemKeys.has('two')).toEqual(true);

    wrapper.find(TreeNode).at(0).simulate('keyDown', { keyCode: KeyCodes.left });
    expect(getStateOf(wrapper).expandedItemKeys.size).toEqual(1);
    expect(getStateOf(wrapper).expandedItemKeys.has('two')).toEqual(true);

    wrapper.unmount();
  });

  it('Shift+F10 should open the contextual menu', () => {
    const menuOpenSelector = `.${TreeAutomationSelectors.openMenuClassName}`;

    const items = getMockItems([{ key: 'one' }, { key: 'two' }]);

    const props = getDefaultProps({
      items: items,
      getItemCommands: () => [
        {
          key: 'New',
          text: 'New',
          iconProps: { iconName: 'Add' }
        },
        {
          key: 'Mail',
          text: 'Mail',
          iconProps: { iconName: 'Mail' }
        }
      ]
    });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(wrapper.find(TreeNode).at(0).find(menuOpenSelector).length).toEqual(0);
    expect(wrapper.find(TreeNode).at(1).find(menuOpenSelector).length).toEqual(0);

    wrapper.find(TreeNode).at(0).simulate('keydown', { keyCode: KeyCodes.f10, shiftKey: true });
    expect(wrapper.find(TreeNode).at(0).find(menuOpenSelector).length).toBeGreaterThan(0);
    expect(wrapper.find(TreeNode).at(1).find(menuOpenSelector).length).toEqual(0);

    wrapper.unmount();
  });

  it('Shift+F10 should not open the contextual menu when disabled', () => {
    const menuOpenSelector = `.${TreeAutomationSelectors.openMenuClassName}`;

    const items = getMockItems([{ key: 'one' }, { key: 'two' }]);

    const props = getDefaultProps({
      items: items,
      getItemCommands: () => [
        {
          key: 'New',
          text: 'New',
          iconProps: { iconName: 'Add' }
        },
        {
          key: 'Mail',
          text: 'Mail',
          iconProps: { iconName: 'Mail' }
        }
      ],
      getItemIsDisabled: () => true
    });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    wrapper.find(TreeNode).at(0).simulate('keydown', { keyCode: KeyCodes.f10, shiftKey: true });
    expect(wrapper.find(TreeNode).at(0).find(menuOpenSelector).length).toEqual(0);

    wrapper.unmount();
  });

  it('TreeNodes should render in the correct order', () => {
    const items: ITestItem[] = getMockItems([
      {
        label: 'one',
        key: 'one',
        children: [
          {
            label: 'two',
            key: 'two',
            children: [{ label: 'three', key: 'three' }]
          },
          { label: 'four', key: 'four' }
        ]
      },
      { label: 'five', key: 'five' }
    ]);
    const props = getDefaultProps({ items, expandedItemKeys: new Set(['one', 'two']) });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(wrapper.find(TreeNode).length).toEqual(5);
    expect(wrapper.find(TreeNode).at(0).props().label).toEqual('one');
    expect(wrapper.find(TreeNode).at(1).props().label).toEqual('two');
    expect(wrapper.find(TreeNode).at(2).props().label).toEqual('three');
    expect(wrapper.find(TreeNode).at(3).props().label).toEqual('four');
    expect(wrapper.find(TreeNode).at(4).props().label).toEqual('five');

    wrapper.unmount();
  });

  it('focus', () => {
    const props = getDefaultProps();
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const instance: any = wrapper.childAt(0).childAt(0).instance();
    expect(instance.focus).toBeDefined();
    const focusZoneFn = jest.fn();
    instance._focusZone.current.focus = focusZoneFn;

    instance.focus();
    expect(focusZoneFn).toHaveBeenCalled();

    wrapper.unmount();
  });

  it('itemIsSelected style should have higher precedence than itemIsSelectable', () => {
    const props = getDefaultProps();
    const selectableColor = 'rgb(170, 0, 0)';
    const selectedColor = 'rgb(0, 170, 170)';

    const wrapper: TreeWrapper = mount(
      <Tree
        {...props}
        styles={{
          itemIsSelectable: {
            backgroundColor: selectableColor
          },
          itemIsSelected: {
            backgroundColor: selectedColor
          }
        }}
        selectedItemKeys={new Set()}
      />
    );

    let firstNode = wrapper.find(TreeNode).at(0).getDOMNode();
    expect(window.getComputedStyle(firstNode).backgroundColor).toEqual(selectableColor);

    wrapper.setProps({ selectedItemKeys: new Set([props.items[0].key]) });

    firstNode = wrapper.find(TreeNode).at(0).getDOMNode();
    expect(window.getComputedStyle(firstNode).backgroundColor).toEqual(selectedColor);
  });

  it('itemIsSelected style should have higher precedence than itemIsNotSelected', () => {
    const props = getDefaultProps();
    const selectableColor = 'rgb(170, 0, 0)';
    const notSelectedColor = 'rgb(0, 170, 170)';

    const wrapper: TreeWrapper = mount(
      <Tree
        {...props}
        styles={{
          itemIsSelectable: {
            backgroundColor: selectableColor
          },
          itemIsNotSelected: {
            backgroundColor: notSelectedColor
          }
        }}
        selectedItemKeys={new Set()}
      />
    );

    const firstNode = wrapper.find(TreeNode).at(0).getDOMNode();
    expect(window.getComputedStyle(firstNode).backgroundColor).toEqual(notSelectedColor);

    wrapper.unmount();
  });

  it('Custom ScrollView', () => {
    const customScrollViewSelector = `.${customScrollViewClassName}`;

    const items = getMockItems([{ key: 'one' }, { key: 'two' }]);

    const props = getDefaultProps({
      items: items,
      scrollView: CustomScrollView
    });
    const wrapper: TreeWrapper = mount(<Tree {...props} />);

    expect(wrapper.find(customScrollViewSelector).length).toEqual(1);
    expect(wrapper.find(TreeNode).length).toEqual(2);

    wrapper.unmount();
  });
});

const customScrollViewClassName = 'custom-scroll-view';
export class CustomScrollView extends React.Component<{ children: React.ReactNode; className: string }> {
  public render() {
    const { children } = this.props;

    return <div className={customScrollViewClassName}>{children}</div>;
  }
}
