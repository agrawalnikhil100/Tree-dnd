...
describe('FilterableComboBox', function () {
    it('should render', function () {
        ...
        var fcb = Enzyme.shallow(React.createElement(FilterableComboBox, { options: options }));
        expect(fcb.exists()).toBeTruthy();
    });
    it('should allow default comboBox behavior when allow.', function () {
        var options = [option('Red'), option('Blue'), option('Green')];
        var onChange = jest.fn();
        var wrapper = Enzyme.mount(React.createElement(FilterableComboBox, { options: options, onChange: onChange, allowFreeform: true }));
    ...
    it('should allow free form when the appropriate option is set to true', function () {
        var options = [option('Red'), option('Blue'), option('Green')];
        var wrapper = Enzyme.mount(React.createElement(FilterableComboBox, { options: options, allowFreeform: true, selectedKey: '' }));
    ...
    describe('filterOptions', function () {
        ...
        it("should only include options where the filter is a case-insensitive substring of the option's text value", function () {
            ...
            expect(filterOptions('bar', options).map(function (op) { return op.text; })).toEqual(['Foo Bar', 'Bar Foo', 'RebAR']);
        });
        it("should include all options underneath a matching header even if the options themselves don't match", function () {
            ...
            expect(filterOptions('blue', options).map(function (op) { return op.text; })).toEqual([
                'Blues',
                'Dark Blue',
                'Azure',
                'Cerulean'
        ...
        it('should include the header of a group with at least one match even if the header does not match', function () {
            ...
            expect(filterOptions('ceru', options).map(function (op) { return op.text; })).toEqual(['Blues', 'Cerulean']);
        });
        it('should strip any trailing divider if actually filtering', function () {
            ...
            expect(filterOptions('blue', options).filter(function (op) { return op.itemType === SelectableOptionMenuItemType.Divider; })
                .length).toBe(0);
        });
    });
});
