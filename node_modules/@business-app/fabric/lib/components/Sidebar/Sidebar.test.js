...
describe('Sidebar', function () {
    ...
    var TestButton = CommandBarButton;
    beforeEach(function () {
    ...
    it('should render the component', function () {
        var wrapper = Enzyme.shallow(React.createElement(Sidebar, { collapsible: false, id: '1', theme: getTheme(), items: [] }));
        expect(wrapper.find('.ba-Sidebar').length).toBe(1);
    });
    it('should render the component with buttons', function () {
        var wrapper = Enzyme.shallow(React.createElement(Sidebar, __assign({}, sidebarButtonExampleProps)));
        expect(wrapper.find(SidebarButton).length).toBe(2);
    });
    it('should render the component with accordion', function () {
        var wrapper = Enzyme.shallow(React.createElement(Sidebar, __assign({}, sidebarAccordionExampleProps)));
        expect(wrapper.find('.ba-SidebarAccordion').length).toBe(2);
    });
    it('should not null ref when clicking accordion', function () {
        var wrapper = Enzyme.mount(React.createElement(Sidebar, __assign({}, sidebarAccordionExampleProps)));
        expect(wrapper.find('.ba-SidebarAccordion').length).toBe(2);
    ...
    it('should render the component using default button', function () {
        var wrapper = Enzyme.shallow(React.createElement(Sidebar, { collapsible: false, id: '1', theme: getTheme(), defaultButton: TestButton, items: [
                {
                    key: 'basic-example-item1',
                    name: 'Item 1',
                    iconProps: { iconName: 'BuildQueue' },
    ...
    it('should fire onCollapseChanged only on setting the collapsed state only if it changes the state', function () {
        var sidebarHandle;
        var testProps = __assign(__assign({}, sidebarAccordionExampleProps), { onCollapseChanged: jest.fn(), componentRef: function (sb) { return (sidebarHandle = sb); } });
        var wrapper = Enzyme.mount(React.createElement(Sidebar, __assign({}, testProps)));
    ...
    it('should dismiss submenu after clicking on an item in the collapsed state', function () {
        ...
        wrapper.instance().setCollapsed(true);
        wrapper.update();
        var contextualMenuButton = wrapper.find(SidebarButton).filter({ title: 'More Stuff Here' });
        var _a = contextualMenuButton.props().menuProps.items[1], onRender = _a.onRender, submenuItem = __rest(_a, ["onRender"]);
        var subMenuButton = Enzyme.shallow(onRender(submenuItem, dismissMock)).find(SidebarButton);
