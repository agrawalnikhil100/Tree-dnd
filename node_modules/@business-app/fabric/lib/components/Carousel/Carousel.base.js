...
var CarouselBase = /** @class */ (function (_super) {
    ...
    CarouselBase.prototype._renderControls = function (classNames) {
        ...
        var tooltipPrevStyles = mergeStyleSets({
            root: ['ba-Carousel-tooltipControl-previous']
        }, tooltipControlStyles);
        var tooltipNextStyles = mergeStyleSets({
            root: ['ba-Carousel-tooltipControl-next']
        }, tooltipControlStyles);
        var controlPrevStyles = mergeStyleSets({
            root: ['ba-Carousel-control-previous']
        }, controlStyles);
        var controlNextStyles = mergeStyleSets({
        ...
        var tooltipControls = function () { return (React.createElement(React.Fragment, null,
            showPrev && (React.createElement(TooltipHost, { content: navPreviousButtonAriaLabel, styles: tooltipPrevStyles },
                React.createElement(IconButton, __assign({ iconProps: { iconName: 'ChevronLeftMed' } }, prevProps)))),
            showNext && (React.createElement(TooltipHost, { content: navNextButtonAriaLabel, styles: tooltipNextStyles },
                React.createElement(IconButton, __assign({ iconProps: { iconName: 'ChevronRightMed' } }, nextProps)))))); };
        var labelControls = function () { return (React.createElement(React.Fragment, null,
            showPrev && (React.createElement(TooltipHost, { styles: tooltipPrevStyles },
                React.createElement(DefaultButton, __assign({ styles: tooltipPrevStyles }, prevProps, { text: prevLabel })))),
            showNext && (React.createElement(TooltipHost, { styles: tooltipNextStyles },
                React.createElement(DefaultButton, __assign({ styles: tooltipNextStyles }, nextProps, { text: nextLabel })))))); };
    ...
    CarouselBase.prototype._renderIndicator = function (classNames, index, content, isSelected) {
        ...
        return disableIndicators ? (React.createElement(IconButton, __assign({}, indicatorProps))) : (React.createElement(IconButton, __assign({}, indicatorProps, { onClick: function () { return _this.goToSlideIndex(index); }, onFocus: function () { return _this.goToSlideIndex(index); } })));
    };
    CarouselBase.prototype._renderIndicators = function (classNames) {
        ...
        return (React.createElement("div", { className: classNames.indicatorsWrapper },
            React.createElement(FocusZone, { direction: FocusZoneDirection.horizontal, isCircularNavigation: true, role: "tablist" }, items.map(function (item, index) {
                var isSelected = index === selectedIndex;
                var _a = tooltipIndicatorsProps[index], id = _a.id, content = _a.content;
                return (React.createElement(TooltipHost, { key: id, id: id, content: content }, _this._renderIndicator(classNames, index, content, isSelected)));
            })),
            !hidePlayButton && (React.createElement(TooltipHost, { content: toggleButtonAriaLabel },
                React.createElement(IconButton, { "aria-label": toggleButtonAriaLabel, iconProps: { iconName: isAutoplayButtonPaused ? 'Play' : 'Pause' }, styles: mergeStyleSets({ root: ['ba-Carousel-play'] }, toggleStyle), onClick: this._togglePlayPause })))));
    };
    ...
    CarouselBase.prototype._updatePlayback = function () {
        ...
        if (!(isAutoplaySuspended || isAutoplayButtonPaused)) {
            this.timer = window.setInterval(function () { return _this.goToNextSlide(); }, delay || TIMER_DELAY_DEFAULT);
        }
    };
    return CarouselBase;
}(React.Component));
